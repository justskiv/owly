...
| action | data |
| --- | --- |
| `create_pool_item` | `{ week, title, hours, category, splittable, source_entity_id?, source_kind? }` |
| `update_pool_item` | `{ week, pool_item_id, ...changed fields }` |
| `delete_pool_item` | `{ week, pool_item_id }` |

`week` — `YYYY-wNN`. `hours` — положительное число (часы; не
минуты). `splittable: true` — задача может быть разрезана между
несколькими блоками; `false` — атомарная (drag целиком).
`source_kind` — `"task" | "project" | "direction" | "ad-hoc"`,
по умолчанию `"ad-hoc"`. `source_entity_id` — `ent-…` или `null`.

`update_pool_item` — id / created_at / updated_at в patch'е
стрипаются (как и в `update_entity`); итоговая форма
валидируется через `PoolItemSchema.parse(merged)`.

`delete_pool_item` каскадно удаляет связанные блоки (по
`pool_item_id`) в той же неделе — на текущей неделе через
schedule store, на прочих неделях напрямую в файле через очередь
записи. Блоки, привязанные к сущности (`source_entity_id`), а не
к pool item, не трогаются.

### Горизонт планирования

| action | data |
| --- | --- |
| `set_horizon_months` | `{ project_id, months: number[] }` |
| `set_horizon_hidden` | `{ project_id, hidden: boolean }` |
| `set_horizon_size` | `{ project_id, size: "big"\|"mid"\|"small" }` |

`months` — массив индексов 0..11 (визуальный диапазон 8 месяцев
от `base_month`). Любой проект, не присутствующий в
`horizon.json`, должен сначала быть добавлен через UI или
прямой write в файл; команды не создают горизонт-стейт сами
(throw `Horizon project ... not found`).

### Каденция направления

| action | data |
| --- | --- |
| `mark_cadence` | `{ direction_id }` |

Shortcut поверх `update_entity`: проставляет
`fields.last_act = today` (локальная YYYY-MM-DD) и сохраняет
все остальные direction-fields (cadence/target/current/...).
Throw'ает, если сущность не найдена или её `type !== "direction"`.

Эквивалент через общий action:

```json
{ "action": "update_entity",
  "data": { "entity_id": "ent-…",
            "fields": { /* весь fields с last_act = today */ } } }
```

`mark_cadence` короче и переносит формирование даты на
сторону приложения (никаких TZ-сюрпризов).
...
### Добавить задачу в пул недели

```json
{
  "id": "cmd-1713015050-create-pool-item",
  "action": "create_pool_item",
  "timestamp": "2026-04-13T15:10:50",
  "data": {
    "week": "2026-w17",
    "title": "Прочитать главу про caching",
    "hours": 2,
    "category": "growth",
    "splittable": true,
    "source_entity_id": null,
...
### Поставить размер проекту на горизонте

```json
{
  "id": "cmd-1713015080-set-horizon-size",
  "action": "set_horizon_size",
  "timestamp": "2026-04-13T15:11:20",
  "data": {
    "project_id": "ent-gc-video-id",
    "size": "big"
  }
}
```

### Отметить каденцию направления (сегодня)

```json
{
  "id": "cmd-1713015090-mark-cadence",
  "action": "mark_cadence",
  "timestamp": "2026-04-13T15:11:30",
  "data": { "direction_id": "ent-japanese-direction-id" }
}
```
...
| Pool item | `pool-<uuid v4>` | `pool-9d8e7f6c-…` |
...
- ❌ `create_block` для несуществующей недели — сначала
  ...
  `Week ... does not exist`.
- ❌ `create_pool_item` для несуществующей недели — pool-файл
  создастся автоматически (per-week), но блоков там не будет
  до `create_week`. Логически связки рвать необязательно.
- ❌ `set_horizon_*` для проекта, которого нет в
  `horizon.json` — кинет `Horizon project ... not found`.
  Сначала добавить проект в горизонт (через UI или ручную
  правку файла).
- ❌ `mark_cadence` на не-direction (task/project/...) —
  кинет `Entity ... is type X, not direction`.
