...
`delete_pool_item` каскадно: на текущей неделе также удаляются
блоки с этим `pool_item_id`. На прошлых/будущих — только сам
item; orphan-блоки в недельных файлах остаются (их можно
почистить отдельным `delete_block`).
