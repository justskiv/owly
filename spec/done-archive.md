...
>
> **Результат после фазы:** в TasksSidebar появляется
> отдельная entry-карточка «✓ Архив выполненных N» (вне
> блоков «Статус / Категория / Приоритет»). Клик открывает
> отдельный экран `Archive` с группами по месяцам завершения,
> быстрыми чипами «Этот месяц / Прошлый месяц / Этот год /
> Всё время», поиском по названию, сортировкой и кнопкой
> «Вернуть в работу» прямо в строке. На Tasks в группе
> «✓ Готово» больше ничего не показываем; кнопка «Выполнено»
> в блоке «Статус» удаляется.

## Контекст

Спека лежит в `spec/done-archive.md` (вне нумерованной последо­
вательности фаз). Все ссылки ниже — от корня репо.

Прочитай:

- `spec/tuzov-os/00-overview.md` — общая картина v2.
- `spec/tuzov-os/v2/phases/03-phase-3-tasks.md` — текущее
  устройство TasksPage, TaskRow, TasksSidebar.
- `spec/tuzov-os/v2/phases/09-phase-9-cleanup.md` — фаза, в
  которой жил пункт «Archive как роадмап» в духе перформанса;
  здесь конкретизируем.
- `spec/tuzov-os/done/01-data-schema.md` §1 — базовые поля
  Entity, в т.ч. `updated_at`, `status`, без `completed_at`.
...
- `src/components/tasks/TasksSidebar.tsx` — текущая раскладка
  faceted-фильтров (`Статус` / `Категория` / `Приоритет`).
  Кнопка «Выполнено» сейчас живёт внутри блока «Статус» и
  выставляет `taskFilter.status = "done"`; в фазе 10 её
  заменяет отдельный entrypoint в архив (см. §2.1, §9.2).
- `src/store/ui.ts` — `TaskFilters` (`{ status, cat, prio }`),
  `TaskStatusFilter` (`"overdue" | "week" | "done"`),
  `setTaskFilterStatus`, `clearTaskFilters`. Удаление `"done"`
  из enum + sidebar-rewire — см. §9.2.
...
кликает entry-карточку «✓ Архив выполненных N» в sidebar'е
→ экран в `<main>` переключается с `TasksPage` на
`ArchivePage`. Активный таб
...
- Trigger перехода в архив: клик по новой entry-карточке
  «✓ Архив выполненных N» в TasksSidebar → `setTasksView(
  "archive")`. Эта карточка — отдельная сущность, **вне**
  блоков «Статус / Категория / Приоритет»: «Выполнено»
  больше не filter-toggle, потому что фильтрация done и
  активных задач разделяется по разным экранам. Удаление
  старого toggle и enum-варианта `"done"` — см. §9.2.
...
**Шаринг хелперов с TaskRow:** `pickAreaTag`, `getAreaColor` —
существуют в `src/services/categories.ts`. **`formatRuDate`
не существует и должен быть создан** новым хелпером в
`src/services/format-date.ts` (или дополнен в существующий
`src/services/calendar-i18n.ts` — там уже есть локаль-
специфичные функции). Сигнатура:

```ts
// "10 мая" если same-year, "10 мая 2025" иначе.
export function formatRuDate(iso: string, now?: Date): string;
```
...
диапазона), restore через чекбокс, новый entrypoint
«✓ Архив выполненных N» в TasksSidebar.

**Что не появляется:** виртуализация, hover-кнопка restore,
кастомный диапазон, чипы периода.

**Что можно потыкать:** Открыть Tasks, кликнуть «✓ Архив
выполненных N» — переход на ArchivePage, видны все
done-задачи сгруппированными по месяцам, ищется,
сортируется. Чек — возврат в work, задача исчезает из
архива. Back-кнопка — возврат на Tasks. На активном
экране Tasks больше нет группы «Готово» снизу.

**Файлы:**

- `src/schemas/entity.ts` — `completed_at` в base.
- `src/services/archive-migration.ts` — backfill скрипт.
- `src/store/entities.ts` — логика `completed_at` в
  `updateEntity`.
- `src/store/ui.ts`:
  - `TaskStatusFilter` без `"done"`.
  - `tasksView` + `setTasksView`.
  - `archiveSearch`, `archiveSort`, `archiveFilter`
    (свой shape, не reuse `taskFilter`), сеттеры.
  - `setPage` сбрасывает `tasksView`.
- `src/services/format-date.ts` (или дополнение в
  `calendar-i18n.ts`) — `formatRuDate` (см. §6.1).
- `src/services/archive-grouping.ts` —
  `groupArchiveByMonth`, `formatGroupLabel`.
- `src/pages/ArchivePage.tsx`.
- `src/components/archive/ArchiveHeader.tsx`,
  `ArchiveToolbar.tsx`, `ArchiveList.tsx`,
  `ArchiveRow.tsx`, `ArchiveGroup.tsx`,
  `ArchiveSidebar.tsx`.
- `src/components/layout/Shell.tsx` — рендер ArchivePage
  при `currentPage === "tasks" && tasksView === "archive"`.
- `src/components/tasks/TasksSidebar.tsx` — изменения
  по §9.2 (убрать «Выполнено» из блока Статус, добавить
  entry-карточку, удалить done-ветку из faceted counts).
- `src/components/tasks/TaskGroups.tsx` — убрать done из
  `GROUP_META`.
- `src/services/group-tasks.ts` — `groupTasks(active)`
  без второго параметра, `TaskGroups` без `done`.
- `src/pages/TasksPage.tsx` — убрать done-ветки в filter
  pipeline (§9.2).
- `src/App.tsx` — вызов `maybeBackfillCompletedAt` в
  `loadAll` (§9.4).
...
`setPage` в `src/store/ui.ts` уже сбрасывает
`horizonHighlight` (для Horizon) и `lastCreatedTaskId` (для
подсветки свежесозданных) — добавляем `tasksView` в тот же
паттерн, **не трогая существующие сбросы**:
...
### 9.2. Удаление `"done"` из faceted-фильтров

После мульти-фильтров TaskFilter — это **не union**, а
интерфейс `TaskFilters = { status, cat, prio }`. Убирается
**одно значение** `"done"` из `TaskStatusFilter` enum
(оставляем `"overdue" | "week"`), и весь связанный код.

**Файлы и изменения:**

- `src/store/ui.ts`:
  - `TaskStatusFilter` без `"done"`.
  - Добавить `tasksView: "active" | "archive"` + setter
    `setTasksView`.
  - Архив-стейт: `archiveSearch: string`, `archiveSort:
    ArchiveSort`, `archiveFilter: { cat, prio, period }` со
    своими сеттерами. Не использовать существующий
    `taskFilter` — у активных и архива разный набор слотов.

- `src/components/tasks/TasksSidebar.tsx`:
  - Удалить кнопку «Выполнено» из блока «Статус» (это
    отдельная вторая `<button className="ts-row">` в
    `<div className="ts-card">` с заголовком «Статус»).
  - Удалить ветку `countWith(..., "done", ...)` и связанные
    case'ы внутри `apply()` (внутренний хелпер useMemo) —
    остаются только `"overdue"`, `"week"`, `null`.
  - Добавить **новый блок-entrypoint вне `Статус/Категория/
    Приоритет`** (отдельный `.ts-card` сверху или снизу
    карточек) — кнопка «✓ Архив выполненных N», где N =
    общее число `status === "done"` задач. Клик →
    `setTasksView("archive")`.

- `src/pages/TasksPage.tsx`:
  - Убрать ветку `pool = status === "done" ? allDone :
    allActive` — `pool = allActive` всегда.
  - Убрать передачу `allDone` в `groupTasks` (см. ниже).
  - Убрать ветку `status === "done" ? groupTasks([], pool) :
    ...` — оставить только `groupTasks(pool)`.
  - `visibleCount` остаётся `pool.length`, передаётся в
    `<TasksHeader count={visibleCount} />` (этот prop уже
    существует, не удалять).

- `src/services/group-tasks.ts`:
  - `groupTasks(active)` — один параметр (done больше не
    нужен).
  - В `TaskGroups` убрать поле `done`. Тесты, которые
    ассертят `groups.done`, обновить.

- `src/components/tasks/TaskGroups.tsx` (`TaskGroupsView`):
  - В `GROUP_META` удалить запись `{ key: "done", icon: "✓",
    label: "Готово", criterion: "выполненные задачи" }`. Импорт
    `TaskGroups` перепроверить — фрешсессия может оставить
    тип, у которого больше нет ключа.
...
строго **до** `Promise.all` boot'а в `loadAll` функции
(`src/App.tsx`, ищи `export async function loadAll`).
Текущая последовательность: `ensureDataDir` →
`maybeMigrateToV2` → `loadConfig` → `Promise.all([
loadEntities, loadWeek, loadRegistry, ...])`. Backfill
встаёт **между** `maybeMigrateToV2` и `loadConfig`:
...
- [ ] При снятии чека с done-задачи `completed_at`
  сбрасывается в `null`.
- [ ] На Tasks в TasksSidebar entry-карточка «✓ Архив
  выполненных N» (вне блоков «Статус / Категория /
  Приоритет») — клик открывает ArchivePage.
...
- [ ] В TasksSidebar нет кнопки «Выполнено» в блоке
  «Статус»; есть отдельная entry-карточка «✓ Архив
  выполненных N», клик ведёт в архив.
- [ ] Faceted counts в TasksSidebar (`countWith`) не
  принимают `"done"` как значение `status` — работают
  только для `null` / `"overdue"` / `"week"`.
- [ ] `task check` (typecheck + vitest + frontend build)
  проходит.
- [ ] Visual baseline для Tasks (`T-7 tasks list visual
  baseline`) перепринят: убрана done-группа, в sidebar
  новая entry-карточка.
...
2. **Завершение активной задачи.** На Tasks отметить чеком
   ...
   время, `status` = `done`.
3. **Переход в архив.** Кликнуть «✓ Архив выполненных N» в
   TasksSidebar. Откроется ArchivePage. t1 видна в группе
   «{Текущий месяц} · этот месяц».
