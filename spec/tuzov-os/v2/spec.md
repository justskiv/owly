...
- **Категория** передаётся через `background: var(--{cat}-fill)` (alpha `.18–.22`, per-cat — bright greens `life`/`health` держим ниже, чтобы не перекрывали warm hues) **плюс** `box-shadow: inset 0 0 0 1px var(--{cat}-edge)` (alpha `.30–.34`). Это даёт vibrant fill + 1px-edge containment без вертикальной полосы слева (CODESTYLE §11). Сами hue-токены (`--work` … `--health`) экспортируются на блоке как `--cat-color` — нужно для status-modifier'ов.
- `padding: 3px 6px`, `font-size: var(--fs-2xs)`, `cursor: grab`.
- `transition: filter .12s, box-shadow .12s, background .12s, opacity .12s`.
- Содержимое:
  - **Заголовок** (`b-title`): `font-weight: 600`, `white-space: nowrap`, `text-overflow: ellipsis`, `color: var(--text-primary)` (без category-tint — двойное кодирование).
  - **Время** (`b-time`): `"09:00–11:00"`, `font-size: 9px`, `color: rgba(255,255,255,.55)`. Показывается только если высота блока > 28px.
...
Каждый блок несёт класс `status-{planned|done|skipped|moved}` (соответствует `BlockStatus`). Все четыре сохраняют hue категории.

- **planned**: дефолт — full vibrant fill + edge.
- **hover** (любой статус): `filter: brightness(1.15)` — лёгкий лифт без layout-shift.
- **done**: `filter: saturate(.55) brightness(.85)` (приглушает hue + чуть темнит, но не убивает цвет как `opacity` на дарке делал; saturate ниже даёт «обугленный» mud-эффект, поэтому держим .55). Заголовок `color: var(--text-tertiary)`, `text-decoration: line-through` (decoration в `--text-disabled`). `b-time` → `--text-disabled`.
- **skipped**: поверх fill ложится `background-image: repeating-linear-gradient(135deg, transparent 0 4px, rgba(255,255,255,.10) 4px 5px)` — диагональная штриховка, читается «отменено» отдельно от done. Title с `line-through` (decoration `rgba(255,255,255,.5)`).
- **moved**: `background-color: transparent`, `border: 1px solid var(--cat-color)` (status-индикатор «призрак» — категория в кольце, не в заливке), `padding: 2px 5px` (компенсация 1px border), `filter: saturate(.6)`. Title `color: var(--text-secondary)`, `font-style: italic`. В правом верхнем углу — `::after { content: '→'; color: var(--cat-color) }`.
- **selected**: `outline: 2px solid var(--accent)`, `outline-offset: 1px`, `z-index: 3`. Композируется со всеми статусами (outline и box-shadow рендерятся параллельно).
...
При правом клике на блоке — контекстное меню (`block-ctx`). Пять действий + ряд категорий-кружков для быстрой смены category одним кликом:

| Action | Label | Icon | Shortcut | Color |
|---|---|---|---|---|
| toggle-done | «Выполнено» / «Не готово» | ✓ | `D` | `--success` |
| toggle-skipped | «Пропущено» / «Не пропущено» | ✗ | `S` | `--text-tertiary` |
| edit | «Редактировать» | — | `Enter` | primary |
| dup | «Дублировать» | — | — | primary |
| del | «Удалить» | — | `⌫` (Backspace/Delete) | `--error` (danger) |

Внизу меню после `ctx-sep` — `ctx-cols` со списком кружков-категорий (`ctx-c`, 14×14, `border-radius: 50%`); кружок текущей категории — `.active` (border-color `--border-strong`, full opacity); клик на кружок применяет category через `update_block`.

`edit` открывает боковой попап редактирования (BlockPopup) у правого края блока. `dup` уважает atomic-pool ссылки (не переносит `pool_item_id` на копию для атомарных), `del` снимает selection.

Стиль контекстного меню:
- `position: fixed`, `z-index: 200`, `background: var(--bg-elevated)`, `border: 1px solid var(--border-hover)`, `border-radius: var(--radius-md)`, `padding: 4px`, `min-width: 190px`, тень `0 8px 24px rgba(0,0,0,.5)`.
- Пункты (`ctx-item`): `display: flex; align-items: center; gap: 8px; padding: 5px 10px; font-size: var(--fs-xs)`, hover/focused `background: var(--bg-tint-1)`. Иконка слева (`ctx-icon`, 14px), label по центру (`ctx-label`, flex:1, ellipsis), shortcut справа (`ctx-sc`, mono, `--text-tertiary`).
- Danger-пункт: `color: var(--error)`, hover/focused `background: rgba(224,104,120,.12)`.

#### Edit popup (двойной клик / Enter в меню)

Боковой попап редактирования (`block-popup`, использует `EntityPopup` shell) у правого края блока. Поля:
- **Заголовок** — inline-input в шапке `ep-task-head`, persists по blur/Enter.
- **Категория** — ряд `f-cats` / `f-cat` кнопок (тонкая капсула с цветной точкой `cd` слева + label). Active — inset border в hue категории.
- **Время** + **Длительность** — два inputs в строке `bp-time-row` (`bp-row > input`), snap к 30 мин на blur.
- **Статус** — `bp-status-toggle`, четыре сегмент-кнопки (Запланировано / ✓ Готово / ✗ Пропущ. / → Перенес.). Active — accent border + tint.
- **Заметки** — `textarea`, `min-height: 48px`, `resize: vertical`, persists по blur.
- **Удалить** — `ep-delete` внизу через `ep-actions`.

Persist-стратегия: каждое поле commit'ит на blur (input/textarea) или сразу (toggle button); при ошибке писка — toast + откат draft к prop value (паттерн `handlePersistError`).

#### Hotkeys

- **Delete / Backspace** на выделенном блоке → удаление.
- В контекстном меню: ↑/↓ — навигация, Enter/Space — applied focus item, `D` — toggle-done, `S` — toggle-skipped, `⌫` / Delete — удалить.
