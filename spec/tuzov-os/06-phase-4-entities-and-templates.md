...
## Технический долг от фазы 2 (сделать здесь же)

Эти пункты привязаны к работе с категориями/тегами, которая делается
именно в фазе 4. Их невыгодно делать раньше — нужен EntityEditor с
выбором тегов и Settings с CRUD по областям, но и откладывать дальше
не стоит: сейчас они уже мешают.

- **Типобезопасность `Block.category` ↔ `Area.id`.** Сейчас
  `category: string` в `BlockSchema` и `id: string` в `AreaSchema`
  — связь стрингово-типизированная. Фаза 4 — естественное место
  это починить:
  - Zod-схема блока валидируется со списком известных area-id (или
    `z.enum(...)` сгенерированный из `config.areas`).
  - На load — блок с неизвестной `category` либо отбрасывается, либо
    мигрируется в дефолтную (логировать).
  - Тот же подход для `entity.tags[]` — теги валидируются по
    `config.areas` + произвольным custom-тегам. Custom-теги
    регистрируются в `config.custom_tags` отдельно.
- **Динамические цвета — не через `var(--${id})` template-string.**
  Сейчас в `WeekSummary`/`DayHeader`/`TimeBlock` цвет категории
  собирается template-string'ом, что молча ломается на typo. После
  типизации category↔area — заменить на `getCategoryColor(id,
  areas): string` helper, цвет передаётся inline (`style={{
  background: color }}`) или через CSS-переменную, заданную из
  config'а. То же место — фиксы для `var(--note-accent)` в Note
  detail (там уже inline через style).
