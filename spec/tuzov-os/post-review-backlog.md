...
### L4. Migrate time-utils на date-fns + DST + ISO weeks ✅ DONE

**Где:** `src/services/time-utils.ts`, `src/services/time-utils.test.ts`
(новый), `vitest.config.ts` (новый), `Taskfile.yml`, `package.json`.

`isoWeekParts`, `getWeekStartDate`, `addWeeks` заменены на тонкие
обёртки над `date-fns` (`getISOWeek`, `getISOWeekYear`,
`startOfISOWeek`, `addWeeks`, `format`). `getCurrentWeekId`,
`getWeekNumber`, `getWeekDates` не трогали — они опираются на
изменённые функции transitively. Удалён внутренний `startOfDay`.

Безопасность: 21 регрессионный тест написан **до** миграции против
текущей реализации (53-недельные года 2020/2026, cross-year
2025-w01 = 2024-12-30, переход 2025-w52 → 2026-w01, roundtrip
dayIndex). Все прошли на старой, все прошли на новой — поведение
идентичное, существующие `data/weeks/*.json` подхватываются.

Добавлены: `vitest@^4.1.5`, `date-fns@^4.1.0`, `task test`,
`task check` теперь включает тесты.

### L5. App icon — проверить mipmap ✅ NO-OP

**Где:** `src-tauri/icons/icon.icns`.

`iconutil -c iconset src-tauri/icons/icon.icns -o /tmp/check.iconset`
показал все 10 стандартных размеров: 16/32/128/256/512 @ 1x и @ 2x.
Source `icon.png` — 1024×1024 RGBA. Mipmap полный. Действий
не требуется.
...
**Где:** `src-tauri/tauri.conf.json:27-29`.

`"csp": null` заменён на baseline strict CSP:
- `default-src 'self'` — всё по умолчанию из своего origin
- `img-src 'self' data: asset: https://asset.localhost` —
  Tauri asset protocol для иконок
- `style-src 'self' 'unsafe-inline'` — Tailwind v4 JIT и
  inline `<style>` в `index.html`
- `font-src 'self' data:` — fontsource woff2
- `script-src 'self'` — bundled JS, без eval
- `connect-src 'self' ipc: http://ipc.localhost` — Tauri IPC

`task dev` smoke прошёл без CSP violations в Web Inspector
console (создание блока, навигация по неделям, переключение
между Planner/Entities/Dashboards, контекстное меню). `'unsafe-eval'`
не понадобился.
...
**PR 5 — cleanup.** ✅ Готово. L1 (✅ NO-OP — версия валидна),
L2 (✅ DONE `cc44344`), L3 (❌ ОТКЛОНЕНО — visible:false убирает
FOUT), L4 (✅ DONE — отдельный коммит с date-fns + vitest).

**Production prep.** L5 (✅ NO-OP — mipmap уже полный),
L7 (✅ DONE — strict CSP включён). L6 (signing + notarization)
— ⏳ заблокирован Apple Developer ID ($99/год), делаем при
подготовке к раздаче build'а.
